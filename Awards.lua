...
function WebDKP_AwardDKP_Event()
    ...
    if reason == "" or reason == nil then
        reason = WebDKP.translations.Noreasonspecified;
    end

    points = tonumber(points);
    if (points == nil or points == "") then
        WebDKP_Print(WebDKP.translations.RewardPoints_Print);
        PlaySound(847);
        return ;
    end
...
function WebDKP_AddDKP(points, reason, forItem, players)
    local date = date("%Y-%m-%d %H:%M:%S", GetServerTime());
    ...
    for k, v in pairs(players) do
        if (type(v) == "table") then
            ...
            if WebDKP_Options["dkpCap"] == 1 and (WebDKP_DkpTable[name]["dkp_" .. tableid] + points > DKPCapVal) then
...
function WebDKP_AddDKPToTable(name, class, points, tableidfrom)
    ...
    if WebDKP_Options["dkpCap"] == 1 then
        local DKPCapVal = tonumber(WebDKP_Options["dkpCapLimit"]);
...
function WebDKP_AwardZeroSum(points, reason, date)
    ...
    reason = WebDKP.translations.reasonZerosum .. reason;
        ...
        if (players == nil) then
            ...
            for k, v in pairs(players) do
                if (type(v) == "table") then
                    ...
                    local reason = WebDKP.translations.reasondecay .. counter;
                    if (not WebDKP_Log[reason .. " " .. date]) then
                        WebDKP_Log[reason .. " " .. date] = {};
                    end
            ...
            WebDKP_Print(WebDKP.translations.hasapplieddecay);
            WebDKP_UpdateTableToShow();
            WebDKP_UpdateTable();
            WebDKP_UpdateLogTable();
        end
