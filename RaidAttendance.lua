...
function WebDKP_UpdateAttendedTable(raid, starttime)
    ...
    table.sort(nameentries, function(a, b)
        return a < b
    end)
    ...
    for i = 1, 25, 1 do
        ...
        if (index <= numEntries) then
            ...
            if WebDKP_DkpTable[charname] ~= nil and WebDKP_DkpTable[charname]["class"] ~= nil then
                ...
                if englishClass ~= nil then
                    local rPerc, gPerc, bPerc, argbHex = GetClassColor(string.upper(englishClass))
                    attendedText:SetTextColor(rPerc, gPerc, bPerc);
                end
            end
...
function WebDKP_RaidStart()
    ...
    local startTime2 = date("%Y-%m-%d %H:%M:%S", GetServerTime());
    local completed = date("%Y%m%d%H%M%S");
    completed = tonumber(completed);
    local date = date("%Y-%m-%d", GetServerTime());
