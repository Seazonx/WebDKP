...
function WebDKP_sign.tohex(s)
    local UINT32 = pcall(string.format, "%08x", 2 ^ 32)
    if UINT32 then
        return format("%08x%08x%08x%08x", str2bei(sub(s, 1, 4)), str2bei(sub(s, 5, 8)), str2bei(sub(s, 9, 12)), str2bei(sub(s, 13, 16)))
    else
        return format("%06x%06x%06x%06x%06x%02x",
                str2bei(sub(s, 1, 3)),
                str2bei(sub(s, 4, 6)),
                str2bei(sub(s, 7, 9)),
                str2bei(sub(s, 10, 12)),
...
function WebDKP_sign.sum(s)
    return WebDKP_sign.new():update(s):finish()
end

function WebDKP_sign.sumhexa(s)
    return WebDKP_sign.tohex(WebDKP_sign.sum(s))
end